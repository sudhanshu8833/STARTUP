...
def send_order(recieved_data,client,quan,price,info,username):
    try:
        ...
        if recieved_data['TT']=='LIMIT':
            p = tradingview_orders(username=username,symbol=symbol, price_in=limit_price,time_in=time.time(),order_type=order_type,transaction_type="LIMIT")
            p.save()

        if recieved_data['TT']=='MARKET':
            p = tradingview_orders(username=username,symbol=symbol, price_in=price, time_in=time.time(),order_type=order_type,transaction_type="MARKET")
            p.save()
        return order1
...
def tradingview_to_brkr(recieved_data,client,info,username):

    
    if recieved_data['BRK']=="BINANCE":
