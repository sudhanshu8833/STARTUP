...
def send_order(recieved_data,client,quan,price,username):
    try:
        ...
        if recieved_data['OT']=='MARKET':
            ...
            response=client.place_order(OrderType=order_type,Exchange=exchange,ExchangeType=product, ScripCode = int(recieved_data['TKN']), Qty=quan, Price=0, iOrderValidity=duration)
        ...
        if recieved_data['OT']=='LIMIT':
            ...
            response=client.place_order(OrderType=order_type,Exchange=exchange,ExchangeType=product, ScripCode = int(recieved_data['TKN']), Qty=quan, Price=limit_price, iOrderValidity=duration)
        ...
        if recieved_data['OT']=='LIMIT':
            p = tradingview_orders(broker="5PAISA",username=username.username,symbol=symbol, Price_in=limit_price,time_in=time.time(),order_type=order_type,transaction_type=transaction_type,quantity=quantity)
            p.save()
        ...
        if recieved_data['OT']=='MARKET':
            p = tradingview_orders(broker="5PAISA",username=username.username,symbol=symbol, Price_in=price, time_in=time.time(),order_type=order_type,transaction_type=transaction_type,quantity=quantity)
            p.save()
